 ...
 * - Edit kernel coefficients in a table with error checking and cell highlighting.
 * - Specify the divisor and offset (or auto-calculate the divisor via a checkbox).
 * - Choose the anchor point (which element of the kernel overlays the processed pixel),
 *   with the anchor cell highlighted.
 * - Quickly preset common convolution filters via preset buttons.
 *
 * When the user clicks "Apply Filter," the widget emits the applyConvolutionFilter signal.
 */
...
public:
     ...
     * @return A QPair where the first element is the X coordinate and the second is the Y coordinate.
     */
...
private:
    ...
    class QCheckBox *checkAutoDivisor;///< Checkbox to auto-calculate the divisor.
